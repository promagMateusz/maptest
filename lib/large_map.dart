...
class MyMapLarge extends ConsumerStatefulWidget {
    const MyMapLarge({Key? key}) : super(key: key);
    
    @override
    ConsumerState<ConsumerStatefulWidget> createState() => _MyMapLarge();
}
GoogleMapController? mapControllerLarge;
CameraPosition goog =  const CameraPosition(target: LatLng(49.031212,-122.861801), zoom: 13);
bool isIdleAfterZoom = false;                    

class _MyMapLarge extends ConsumerState<MyMapLarge>{
  @override
  Widget build(BuildContext context) {
    ...
    mapControllerLarge?.moveCamera(CameraUpdate.newCameraPosition(
        currentPosition.currentPosition,
    ));
    
    Fluster<MapMarker> cluster = MapHelper.initClusterManager(markers.getMarkers, 0, 20);    
    print('hi');
    return SizedBox( 
        child: GoogleMap(
            ...
            markers: MapHelper.getGoogleMapsMarkers(cluster, currentPosition.currentPosition).toSet(),
...
LatLng centeredCameraPosition(ref){
    ...
    if (latLngSet.isEmpty == true){ // Change this to something else later
        return const LatLng(49.031212, -122.861801);
    }
    double totalLat = 0;
    double totalLong = 0;
    for (var value in latLngSet) {totalLat += value.latitude; totalLong += value.longitude;}
